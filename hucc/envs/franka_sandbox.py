...
class FrankaRodSandbox(MujocoEnv, utils.EzPickle):
    """
    franka rod sandbox environment with only table
    ...
    def __init__(self, frame_skip: int = 10, random_init: bool = False):
        # TODO: toggle for box in scene or not
        utils.EzPickle.__init__(**locals())
        self._steps = 0
        # TODO: if box in scene, use this instead
        ...
        self.init_qvel_box_pushing = np.zeros(9)
        ...
        MujocoEnv.__init__(
            ...
            model_path=os.path.join(
                os.path.dirname(__file__), "assets", "franka_rod_sandbox.xml"
            ),
    ...
    def step(self, action):
        ...
        resultant_action = np.clip(
            action + self.data.qfrc_bias[:7].copy(), -q_torque_max, q_torque_max
        )
        ...
        if not unstable_simulation:
            reward = self._get_reward(rod_tip_pos, rod_quat, qpos, qvel, action)
        else:
            reward = -50
        ...
        obs = self._get_obs()
        infos = {"episode_energy": self._episode_energy, "num_steps": self._steps}
    ...
    def reset_model(self):
        ...
        # TODO: if box in scene, use this
        # self.data.joint("box_joint").qpos = box_init_pos

        # TODO: random position above desk if box not in sandbox
